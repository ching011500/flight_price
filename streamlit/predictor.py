...
# 2. 抵達機場（動態，根據出發機場篩選）
# 合併所有真實組合，用以篩選屬於此 departure 的抵達機場
...
arr_disp    = [arrival_mapping[a] for a in arrivals if a in arrival_mapping]
...
if st.button('🔍 查詢預測票價', key="search_btn"):
    ...
    if res.empty:
        ...
        # 將預測區間（下界~上界）組合成一個欄位
        res['預測區間'] = res['預測區間下界'].round().astype(int).astype(str) + " ~ " + res['預測區間上界'].round().astype(int).astype(str)
        # 假期文字替換
        holiday_map = {0:'無假期',1:'短假期',2:'長假期'}
        res['假期'] = res['假期'].map(holiday_map)
        # 是否為平日文字替換
        weekday_map = {0:'平日',1:'假日'}
        res['是否為平日'] = res['是否為平日'].map(weekday_map)

        # 根據模式選顯示欄位
        if mode=='long':
            disp_cols=['出發機場代號','抵達機場代號','出發時段','抵達時段',
                       '艙等','停靠站數量','航空聯盟','機型分類','假期','是否為平日',
                       '航空聯盟組合','航空公司組合','飛行時間兩段分類',
                       '停留時間_分鐘','實際飛行時間_分鐘','competing_flights','預測區間']
        else:
            disp_cols=['出發機場代號','抵達機場代號','出發時段','抵達時段',
                       '艙等','航空聯盟','機型分類','假期','是否為平日',
                       '飛行時間_分鐘','competing_flights','預測區間']
